...
const getIBCTokenDenom = (denom: string): string =>
    denom.replace(/ibc/gi, 'ibc/');
...
export const addLiquidity = async (
    ...
    const data = req.body;
    const requiredValidationResult = validateRequired(data, [
        'tokenA',
        'tokenB',
        'contractAddress',
        'stakingAddress',
...
export const editLiquidity = async (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    ...
    Liquidity.findOne(
        { tokenA: getIBCTokenDenom(tokenA), tokenB: getIBCTokenDenom(tokenB) },
        (err, doc) => {
            if (err) return next(err);
            if (!doc)
                return res.status(400).send({
...
export const getAllLiquidities = async (
    _req: Request,
    res: Response,
    next: NextFunction,
) => {
...
export const backupLiquidities = async (
    req: Request,
    res: Response,
    _next: NextFunction,
) => {
    ...
    try {
        for (let liquidity of data) {
            ...
            try {
                const requiredValidationResult = validateRequired(liquidity, [
                    'tokenA',
                    'tokenB',
                    'contractAddress',
                ]);
                ...
                const existingData = await getDataFromDB(Liquidity, {
                    tokenA,
                    tokenB,
                });
